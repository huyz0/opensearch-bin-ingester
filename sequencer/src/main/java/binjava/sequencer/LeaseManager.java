 ...
 *
 * <p>⚠️ TTL and the renew interval are CONFIGURATION — see {@link LeaseConfig},
 * which owns them and says why they are not constants.
 */
public final class LeaseManager {
    ...
    public LeaseManager(BinStore store, LeaseConfig config, Clock clock) {
        ...
        // ⚠️ Unpacked into fields rather than read through `config`, and the
        // reason is immutability rather than call count: every field here is
        // final and assigned once from an immutable record, so there is no
        // state that can drift out of step with the config. `key` has to be
        // materialised once in any case, or `String.format` would re-run at
